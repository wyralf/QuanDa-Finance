...
import com.tome.im.adapter.ViewsPagerAdapter;
import com.tome.modulebase.utils.DensityUtils;
...
public class ChatExtendMenu extends ViewPager {
    ...
    private void init(Context context, AttributeSet attrs){
        ...
        numColumns = ta.getInt(R.styleable.ChatExtendMenu_numColumns, 4);
        ta.recycle();
    }
    ...
    public void init(){
        ...
        if(totalSize != 0){
            ...
            for(int i = 0; i < pageCount; i++){
                GridView gridView = new ExpandGridView(context);
                gridView.setNumColumns(numColumns);
                gridView.setStretchMode(GridView.STRETCH_COLUMN_WIDTH);
                gridView.setGravity(Gravity.CENTER_VERTICAL);
                gridView.setVerticalSpacing(DensityUtils.dip2px(context, 8));
            ...
            setAdapter(new ViewsPagerAdapter(views));
    ...
    private class ItemAdapter extends ArrayAdapter<ChatMenuItemModel> {
        ...
        public ItemAdapter(Context context, List<ChatMenuItemModel> objects) {
            super(context, R.layout.chat_menu_item, objects);
            this.context = context;
        }
